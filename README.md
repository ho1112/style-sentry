...
Style Guard is a custom CSS linter and VSCode extension designed to enforce your team's design system and coding standards.

---

## Table of Contents
- [English](#style-guard)
- [한국어 (Korean)](#style-guard-한국어)
- [日本語 (Japanese)](#style-guard-日本語)
...
To use the Style Guard CLI, install it globally from the project root:
...
1.  Navigate to the `style-guard-vscode` directory.
2.  Run `npm install` to install dependencies.
3.  Run `npm run compile` to compile the TypeScript code.
4.  In VSCode, open the Extensions view, click the `...` menu, select `Install from VSIX...`, and choose the `style-guard-vscode-0.0.1.vsix` file.

### Configuration

Style Guard is configured using a `.styleguardrc.js` file in the root of your project. Here is an example configuration:
...
To run the linter from the command line, execute the `style-guard` command in your project's root directory:
...
Style Guard CLI를 사용하려면, 프로젝트 루트에서 다음 명령어로 전역 설치합니다:
...
1.  `style-guard-vscode` 디렉토리로 이동합니다.
2.  `npm install` 명령으로 의존성을 설치합니다.
3.  `npm run compile` 명령으로 TypeScript 코드를 컴파일합니다.
4.  VSCode의 확장 프로그램 뷰에서 `...` 메뉴를 클릭한 후, `VSIX에서 설치...`를 선택하여 `style-guard-vscode-0.0.1.vsix` 파일을 설치합니다.

### 설정

Style Guard는 프로젝트 루트의 `.styleguardrc.js` 파일을 통해 설정합니다. 아래는 설정 예시입니다:
...
커맨드 라인에서 린터를 실행하려면, 프로젝트 루트 디렉토리에서 `style-guard` 명령을 실행하세요:
...
Style Guard CLIを使用するには、プロジェクトのルートで次のコマンドを実行してグローバルにインストールします:
...
1.  `style-guard-vscode` ディレクトリに移動します。
2.  `npm install` コマンドで依存関係をインストールします。
3.  `npm run compile` コマンドでTypeScriptコードをコンパイルします。
4.  VSCodeの拡張機能ビューで `...` メニューをクリックし、「VSIXからインストール...」を選択して `style-guard-vscode-0.0.1.vsix` ファイルをインストールします。

### 設定

Style Guardは、プロジェクトのルートにある `.styleguardrc.js` ファイルで設定します。以下は設定例です:
...
コマンドラインからリンターを実行するには、プロジェクトのルートディレクトリで `style-guard` コマンドを実行します:
